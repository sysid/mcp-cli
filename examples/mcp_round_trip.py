...
5. Prints the assistant's raw reply
6. Parses & executes any function calls via MCP stdio processor
7. Prints the function results
"""
...
async def main() -> None:
    ...
    mcp_tools = []
    original_to_openai_name = {}  # Mapping to convert back later
        ...
        for tc in tool_calls:
            if tc.get("function") and "name" in tc.get("function", {}):
                ...
                if openai_name in original_to_openai_name:
                    ...
                    print(f"Unknown tool name: {openai_name}")
    elif not reply:
