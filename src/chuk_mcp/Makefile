...
help:
	...
	@echo "  build         Build source + wheel dists using \`$(PYTHON3)\`"
	@echo "  test          Run tests with uv"
	@echo "  clean         Remove build artifacts"
	@echo "  publish       Publish to PyPI (uses \`$(PYTHON3)\` + twine)"
	@echo "  publish-test  Publish to TestPyPI (uses \`$(PYTHON3)\` + twine)"
	@echo
